...
def parse_options():
    ...
    parser.add_option('--scylla_host', action='store',
                      help='ScyllaDB host/contact point')
    parser.add_option('--scylla_port', action='store', default='9042',
                      help='ScyllaDB native transport port (default: 9042)')
    parser.add_option('--scylla_keyspace', action='store',
                      help='Keyspace to use (and create if needed)')
    parser.add_option('--scylla_username', action='store',
                      help='Username to connect (optional)')
    parser.add_option('--scylla_password', action='store',
    ...
    if (options.postgres_server is None or
        ...
        options.scylla_host is None or
        options.scylla_port is None or
        options.scylla_keyspace is None):
...
def main():
    ...
    try:
        ...
        replaces = {'@PSCHEMANAME': args.postgres_schema,
                    ...
                    '@SHOST': args.scylla_host,
                    '@SPORT': args.scylla_port,
                    '@SUSER': args.scylla_username if args.scylla_username else '',
                    '@SPASSWORD': args.scylla_password if args.scylla_password else '',
                    '@SKEYSPACE': args.scylla_keyspace}
