...
def check_ver(conn, min_ver, max_ver, dbtype):
    ...
    dbtype  -- A string with the database type (postgresql|mssql|scylla)
    """
    cursor = conn.cursor()
...
def get_logs_path(conn, dbtype):
    ...
    Keyword arguments:
    conn  -- A db connection (according to Python DB API v2.0)
    dbtye -- A string with the database type (postgresql|mssql|scylla). mssql and scylla will return an empty a array.
    """
    logs = []
    if dbtype == 'mssql' or dbtype == 'scylla':
        return(logs)
    cursor = conn.cursor()
    try:
