...
def version_to_array(version, dbtype):
    ...
    dbtype  -- A string with the database type (postgresql|scylla)
    """
    if version != '':
        try:
...
def check_ver(conn, min_ver, max_ver, dbtype):
    ...
    if dbtype == 'postgresql':
        ...
        sentence = 'SELECT release_version FROM system.local;'
...
def get_logs_path(conn, dbtype):
    ...
    Keyword arguments:
    conn  -- A db connection (according to Python DB API v2.0)
    dbtye -- A string with the database type (postgresql|scylla). scylla will return an empty array.
    """
    logs = []
    if dbtype == 'scylla':
        return(logs)
    ...
    for fname in files:
        ...
        if check_ver(conn, req_ver['min'], req_ver['max'], dbtype):
            ...
            try:
                ...
                cursor.execute(sentence)
                conn.commit()
                # Print any messages (notices, debug, ScyllaDB messages from scylla_fdw, etc.)
                # Note: only psycopg2 has notices attribute, not scylla-driver
                if hasattr(conn, 'notices') and conn.notices:
                    for notice in conn.notices:
                        # Strip whitespace and print
                        print_info(notice.strip())
                    conn.notices.clear()
                cursor.close()
                tests['ok'] += 1
            except Exception as e:
